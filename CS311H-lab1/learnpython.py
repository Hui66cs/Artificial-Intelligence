...
# Note that a tuple of length one has to have a 逗号 after the last element but
# tuples of other lengths, even zero, do not.
type((1))   # => <class 'int'> 默认当成普通括号，是整数
type((1,))  # => <class 'tuple'> 是元组
type(())    # => <class 'tuple'>
...
invalid_dict = {[1,2,3]: "123"}  # => Yield a TypeError: unhashable type: 'list'。列表可变
...
# 字典解包，将一个字典的键值对解包到另一个字典中。若两个字典有同样的键，则以后来的为准
{"a": 1, **{"b": 2}}  # => {'a': 1, 'b': 2}
{"a": 1, **{"a": 2}}  # => {'a': 2}
...
invalid_set = {[1], 1}  # => Raises a TypeError: unhashable type: 'list' 集合元素不可修改
valid_set = {(1,), 1}
...
try:
    ...
    raise IndexError("This is an index error") #手动模拟了一个错误
except IndexError as e:
    pass                 # pass直接跳过了，应该要加提示
...
filled_dict = {"one": 1, "two": 2, "three": 3}
our_iterable = filled_dict.keys()  #相当于创建了一个可迭代视图（会动态变化），节省内存
...
class Human:
    ...
    # self在调用时不用显式写
    def __init__(self, name):  #类似构造函数，但内存分配要先__new__,创建好了实例再__init__初始化
...
class Superhero(Human):  #括号里为父类名字
